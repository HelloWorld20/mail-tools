...
module.exports = {
	...
	yjfzzyID: '14102',	
	tplName: '上传邮件模板时需要用到的模板名称。',		//上传邮件模板时需要用到的模板名称。
	confName: '上传邮件模板时需要用到的封装资源名称。', 	//上传邮件模板时需要用到的封装资源名称。
	apiType: 'bill',	//投递方式：ng：NGBOSS邮件下发；bill：账单类型；api_0：api接口且infoType为0；api_2：api接口且infoType为2；
	sourceFileName: '',	//数据文件名
	
	uploadHtml: '待上传的HTML文件路径',		//待上传的HTML文件路径
	uploadQvga: '待上传的Qvga文件路径',		//待上传的Qvga文件路径
